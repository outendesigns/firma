...
function initMap() {
  ...
  markerArray.forEach(function(marker, i){
    ...
    marker.addListener('click', function(){
      ...
      if(locationArray[i].image == ""){
        var newContent = "<span style='font-family:futura,sans-serif; font-size:16px;'><p>"+locationArray[i].name+"<br>"+locationArray[i].city+"</p></span>"
      } else {
        var newContent = "<span style='font-family:futura,sans-serif; font-size:16px;'><p>"+locationArray[i].name+"<br>"+locationArray[i].city+"</p><img src='"+locationArray[i].image+"' width='150' height='150'><br><a href='"+locationArray[i].link+"' target='_blank'>More Info</a></span>"
      }
      map.setZoom(11);
...
function hideLegend(){
  ...
  document.getElementById("hideLegend").className = "mapToggle toggleActive";
  document.getElementById("showLegend").className = "mapToggle";
}
function showLegend(){
  document.getElementById("legend").style.display="block";
  document.getElementById("hideLegend").className = "mapToggle";
  document.getElementById("showLegend").className = "mapToggle toggleActive";
}
...
function centerOnClick(lt,lg,id){
  ...
  if(locationArray[id].image == ""){
    var newContent = "<span style='font-family:futura,sans-serif; font-size:16px;'><p>"+locationArray[id].name+"<br>"+locationArray[id].city+"</p></span>"
  } else {
    var newContent = "<span style='font-family:futura,sans-serif; font-size:16px;'><p>"+locationArray[id].name+"<br>"+locationArray[id].city+"</p><img src='"+locationArray[id].image+"' width='150' height='150'><br><a href='"+locationArray[id].link+"' target='_blank'>More Info</a></span>"
  }
  map.setZoom(10);
  map.panTo(centerLatLng);
  infowindow.setContent(newContent);
...
//Accordion the Project List//
function displayProjList(id,button,title){
  var x = document.getElementById(id);
  if (x.className.indexOf("show") == -1) {
    x.className += " show";
    document.getElementById(button).innerHTML = title+" &#9650;";
  } else { 
    x.className = x.className.replace(" show", "");
    document.getElementById(button).innerHTML = title+" &#9660";
  }
}
